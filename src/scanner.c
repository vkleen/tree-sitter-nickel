...
unsigned serialize(Scanner *scanner, char *buffer) {
    ...
    if (scanner->expected_percent_count.len >=
...
void deserialize(Scanner *scanner, const char *buffer, uint8_t length) {
...
bool valid_symtag_start(int32_t lookahead) {
    return ('a' <= lookahead && lookahead <= 'z') ||
...
bool valid_symtag_middle(int32_t lookahead) {
    return valid_symtag_start(lookahead) ||
           ('0' <= lookahead && lookahead <= '9') || lookahead == '-' ||
...
// (`some-prefix-s%"`).
bool scan_multstr_start(Scanner *scanner, TSLexer *lexer) {
...
bool scan_multstr_end(Scanner *scanner, TSLexer *lexer) {
...
bool scan_str_start(Scanner *scanner, TSLexer *lexer) {
...
// Precondition of this function is that the lookahead is '"'
bool scan_str_end(Scanner *scanner, TSLexer *lexer) {
...
bool scan_interpolation_start(Scanner *scanner, TSLexer *lexer) {
...
// Precondition of this function is that the lookahead is '"'.
bool scan_quoted_enum_tag_start(Scanner *scanner, TSLexer *lexer) {
